...
export default function CustomerDetailsHome() {
  ...
  const { room, home, checkInDate, checkOutDate, guests, totalPrice, nights } = route.params || {};
  ...
  const handleProceedToPayment = () => {
    ...
    navigation.navigate('PaymentDetailsHome', {
      room,
      home,
      checkInDate,
      checkOutDate,
    ...
    <SafeAreaView className="flex-1 bg-gray-100">
      ...
      <ScrollView className="flex-1 p-4" contentContainerStyle={{ paddingBottom: 160 }}>
        ...
        <View className="mb-6 rounded-xl bg-white p-4 shadow-lg">
          ...
          {/* Home Info with Image */}
          <View className="flex-row mb-4">
            <Image 
              source={{ uri: room?.images?.[0] || home?.image || 'https://images.unsplash.com/photo-1564501049412-61c2a3083791?ixlib=rb-4.0.3&auto=format&fit=crop&w=800&q=80' }} 
              className="w-20 h-20 rounded-lg mr-4"
            />
            <View className="flex-1 justify-between">
              <View>
                <Text className="text-lg font-bold text-gray-800">{home?.name || room?.location || 'Luxury Home'}</Text>
                <Text className="text-gray-600 text-sm">{home?.location || room?.location || 'Beautiful Location'}</Text>
              </View>
              <View className="flex-row items-center">
                <View className="flex-row items-center bg-blue-100 px-2 py-1 rounded">
                  <Ionicons name="star" size={14} color="#f59e0b" />
                  <Text className="text-amber-500 text-sm ml-1">{room?.rating || home?.rating || 4.5} Excellent</Text>
                </View>
              </View>
            </View>
          </View>
          ...
          <View className="border-t border-gray-200 pt-3">
            <View className="flex-row justify-between mb-2">
              ...
              <Text className="text-gray-800 font-medium">{room?.title || 'Deluxe Room'}</Text>
            </View>
            <View className="flex-row justify-between mb-2">
