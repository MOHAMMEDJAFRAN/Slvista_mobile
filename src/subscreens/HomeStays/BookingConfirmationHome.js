...
export default function BookingConfirmationHome() {
  ...
  const { room, home, totalPrice, customerDetails, nights, checkInDate, checkOutDate } = route.params || {};
            ...
            <View className="flex-1 justify-between">
              <View>
                ...
                <Text className="text-gray-600 text-sm mt-1">
                  {home?.name || 'Luxury Home'}
                </Text>
                <Text className="text-gray-500 text-sm mt-1">
                  {home?.location || room?.location || 'Beautiful Location'}
                </Text>
              </View>
              <View className="flex-row items-center mt-2">
                <Ionicons name="star" size={16} color="#f59e0b" />
                <Text className="text-amber-500 text-sm ml-1">
                  {room?.rating || home?.rating || 4.8} • {room?.reviews || home?.reviews || 147} Reviews
                </Text>
              </View>
            </View>
          </View>
