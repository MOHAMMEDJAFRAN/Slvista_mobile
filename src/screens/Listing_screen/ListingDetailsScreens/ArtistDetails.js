...
const ArtistDetails = () => {
  ...
  const images = artistItem.images || [artistItem.image];
  
  // Sample reviews data with user images
  const reviews = artistItem.reviews || [
    {
      id: 1,
      user: "Sarah Johnson",
      rating: 5,
    ...
    },
    {
      ...
      user: "Emma Wilson",
      rating: 5,
      comment: "Absolutely loved the workshop! The instructor was talented and engaging.",
      date: "2023-10-10",
    ...
    },
    {
      ...
      user: "Michael Brown",
      rating: 4,
      comment: "Wonderful experience. Would highly recommend to anyone interested in art.",
      date: "2023-10-08",
  ...
  // Languages
  const languages = artistItem.languages || ["English", "Tamil", "Sinhala"];
  ...
  const shareArtist = async () => {
    try {
      await Share.share({
        message: `Check out ${artistItem.name} - ${artistItem.category} artist. ${artistItem.website || ''}`,
        url: artistItem.website,
  ...
  const openLocationInMaps = () => {
    const encodedLocation = encodeURIComponent(artistItem.location);
      ...
      <ScrollView className="flex-1" showsVerticalScrollIndicator={false}>
        ...
        <View className="p-5 bg-white rounded-t-3xl -mt-6">
          <View className="flex-row justify-between items-start">
            <View className="flex-1">
              <Text className="text-2xl font-bold text-gray-800">{artistItem.name}</Text>
              <View className="flex-row items-center mt-2">
                <FontAwesome 
                  name={categoryIcons[artistItem.category]} 
                  size={16} 
                  color="#006D77" 
                />
                <Text className="text-[#006D77] text-sm ml-2 font-medium">{artistItem.category}</Text>
              </View>
            </View>
            <View className="bg-[#E6F6F8] px-3 py-1 rounded-full">
              {renderPriceRange(artistItem.price)}
            </View>
          </View>
          
          <View className="mt-4">
            {renderStars(artistItem.rating)}
          </View>
          ...
          <View className="mt-4 p-3 bg-gray-50 rounded-xl">
            <View className="flex-row items-center">
              ...
              <Text className="text-gray-800 font-medium ml-2">{artistItem.location}</Text>
            </View>
            <View className="flex-row items-center mt-2">
              <Ionicons name="brush" size={20} color="#006D77" />
              <Text className="text-gray-800 font-medium ml-2">{artistItem.specialty}</Text>
            </View>
          </View>
          ...
          <View className="flex-row justify-between mt-6 mb-2">
            <TouchableOpacity 
              ...
              onPress={() => Linking.openURL(`tel:${artistItem.phone}`)}
            >
            ...
            <TouchableOpacity 
              className="flex-1 bg-[#E6F6F8] p-3 rounded-xl mx-1 flex-row items-center justify-center"
              onPress={() => Linking.openURL(`mailto:${artistItem.email}`)}
            >
            ...
            {artistItem.website && (
              <TouchableOpacity 
                className="flex-1 bg-[#E6F6F8] p-3 rounded-xl mx-1 flex-row items-center justify-center"
                onPress={() => Linking.openURL(artistItem.website)}
              >
          ...
          >
            ...
            {(expandedSection === 'social' || expandedSection === null) && (
              <View className="mt-3 flex-row flex-wrap">
                {artistItem.socialMedia?.instagram && (
                  <TouchableOpacity 
                    className="bg-[#E6F6F8] p-3 rounded-xl mr-2 mb-2 flex-row items-center"
                    onPress={() => Linking.openURL(`https://instagram.com/${artistItem.socialMedia.instagram.replace('@', '')}`)}
                  >
                    <Ionicons name="logo-instagram" size={20} color="#E1306C" />
                    <Text className="text-[#006D77] font-medium ml-2">Instagram</Text>
                  </TouchableOpacity>
                )}
                {artistItem.socialMedia?.facebook && (
                  <TouchableOpacity 
                    className="bg-[#E6F6F8] p-3 rounded-xl mr-2 mb-2 flex-row items-center"
                    onPress={() => Linking.openURL(`https://facebook.com/${artistItem.socialMedia.facebook}`)}
                  >
                    <Ionicons name="logo-facebook" size={20} color="#4267B2" />
                    <Text className="text-[#006D77] font-medium ml-2">Facebook</Text>
                  </TouchableOpacity>
                )}
                {artistItem.socialMedia?.youtube && (
                  <TouchableOpacity 
                    className="bg-[#E6F6F8] p-3 rounded-xl mr-2 mb-2 flex-row items-center"
                    onPress={() => Linking.openURL(`https://youtube.com/${artistItem.socialMedia.youtube}`)}
                  >
            ...
            {expandedSection === 'cancellation' && (
              <View className="mt-3">
                <Text className="text-gray-600 text-sm leading-6">
                  {artistItem.cancellationPolicy || "Free cancellation up to 24 hours before the scheduled performance. Cancellations within 24 hours will incur a 50% fee. No-shows will be charged the full amount."}
                </Text>
              </View>
            )}
          </TouchableOpacity>
          ...
          >
            ...
            {expandedSection === 'history' && (
              <View className="mt-3">
                <Text className="text-gray-600 text-sm leading-6">
                  {artistItem.performanceHistory || "Experienced artist with over 5 years of professional performance. Has performed at various cultural events, weddings, and corporate functions. Specializes in traditional and contemporary performances."}
                </Text>
              </View>
            )}
          </TouchableOpacity>
      ...
      <View className="p-5 bg-white border-t border-gray-200">
        <View className="flex-row">
          ...
          <TouchableOpacity 
            ...
            onPress={() => Linking.openURL(`https://wa.me/${artistItem.phone.replace(/\D/g, '')}`)}
          >
